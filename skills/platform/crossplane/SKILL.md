...
name: crossplane
description: >
  Compose Crossplane v2 XRDs and Compositions. Use when writing a Crossplane
  module, creating a CompositeResourceDefinition, authoring a Composition
...
1. **Design the XR schema** — required fields sparingly, prefer arrays over scalars, prefer enums over booleans (except for debug/feature toggles), leave room for engine variants. See [references/xrd-anatomy.md claim rules](references/xrd-anatomy.md).
2. **Write the XRD** (`apiextensions.crossplane.io/v2`, `scope: Namespaced`) and the **Composition** (`apiextensions.crossplane.io/v1`, `mode: Pipeline`) — see §1 and §2 below. For full pipeline anatomy and function constraints, see [references/composition-anatomy.md](references/composition-anatomy.md).
3. **Find the managed resource** for your provider. Namespaced v2 MRs are `<service>.<provider>.m.upbound.io/v1beta1`. Discovery: `kubectl get mrd`, `kubectl explain`, or the Upbound marketplace. Schema dump: `scripts/get-crd-field.sh <crd>`. See [references/mrd-discovery.md](references/mrd-discovery.md) and the provider-specific reference for CRD group conventions (e.g. [AWS](references/aws.md)).
...
- **SSA validates existing XRs against the updated XRD schema** — changing an XRD field type (e.g. `items: string` → `items: object`) fails on existing XRs: Kubernetes CRD validation doesn't re-validate existing objects, but the composition controller's SSA does. Old XRs enter `ReconcileError` with `expected map, got string` because SSA rejects every status update. **Fix: add `anyOf` to the XRD for safe migration.** Accept both old and new types:
  ```yaml
  vault_paths:
    type: array
    items:
      anyOf:
        ...
        - type: object
          ...
          properties:
            ...
            # ... new fields
  ```
  Deploy the `anyOf` XRD first. Update all XRs to the new format (apply directly, ArgoCD may not propagate due to SSA conflicts). Remove the old type from `anyOf` once all XRs are migrated. See `references/xrd-anatomy.md` `anyOf` pattern.
...
- **AWS**: [references/aws.md](references/aws.md) — CRD group convention (`<service>.aws.m.upbound.io/v1beta1`), providerConfigRef patterns, default tags, module path conventions (`modules/aws/`), complete examples (XNetwork VPC, XPostgres RDS).
- **Vault**: *(coming soon)* — follow the same pattern as `aws.md`.
