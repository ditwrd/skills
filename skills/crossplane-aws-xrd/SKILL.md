...
description: >
  ...
  discovering a provider-aws ManagedResourceDefinition schema, or testing
  locally with xprin.
---
...
- **Deterministic names** — set `metadata.name` on every composed resource from a stable field on the XR. Set `crossplane.io/external-name` only when the provider does NOT derive its own — some providers overwrite it (SQS Queue → queue URL, S3 BucketNotification → bucket name), and setting it in the template creates a perpetual diff that causes reconciliation thrashing (see next rule). When in doubt, omit `external-name` and check after first reconcile. Also set `forProvider.name` when the CRD supports it — `external-name` is for Crossplane tracking only; upjet/terraform uses `forProvider.name` for the actual AWS resource name. Missing `forProvider.name` causes terraform to auto-generate a random resource name. Random names break re-render diffing and `resourceRefs`.
:
- **Reconciliation thrashing / circuit breaker** — if an XR's status shows `WatchCircuitOpen` ("Too many watch events"), a managed resource is being updated in a tight loop. Root cause: perpetual diff between what the template renders and what the provider writes back. Diagnose: (a) find the MR with high `metadata.generation` (100+ within minutes); (b) diff `spec.forProvider` against template output — look for fields the provider sets but the template omits; (c) check Crossplane logs: `kubectl logs -n crossplane-system deployment/crossplane | grep -i circuit`. Common causes: `crossplane.io/external-name` set to a value the provider overwrites, provider-resolved `*Ref` fields written back to `spec.forProvider` (e.g. `queueArnRef` → `queueArn`), or provider default tags omitted (see next rule). Fix: either include the provider-written field in the template (read from observed resources), or stop setting the field the provider manages.
