...
:- **Pipeline must end with `function-auto-ready`**. Render step(s) precede it. The standard 2-step shape is render-then-auto-ready. For cross-resource dependencies, prefer a single render step with `*Ref` fields (queueUrlRef, queueArnRef) — Crossplane resolves ordering, and the composition emits everything unconditionally. Use `.observed.resources` with `getResourceCondition` for conditional content (e.g. Deny→Allow policies based on readiness). Multi-step pipelines are an alternative when dependency can't be expressed via `*Ref` alone.
...
:- **Conditional resource policies** — when a resource's policy content depends on another resource's Ready state (e.g. QueuePolicy Deny→Allow based on Queue readiness): look up the observed resource with `(index $.observed.resources "key")`, check readiness with `getResourceCondition "Ready" $observed`, read ARN/id with `dig "resource" "status" "atProvider" "arn" "" $observed`, then emit `"Effect": "{{ if $ready }}Allow{{ else }}Deny{{ end }}"` with `"Resource": "{{ if $ready }}{{ $arn }}{{ else }}*{{ end }}"`. `Resource: "*"` in a resource-based policy means "this resource" — safe for Deny defaults.
